...
mutable struct ControlInputs{T}
    uc::Vector{T}
end
...
function nominal_system(sys, ccm; Ts = 0.01)
    ccm_f = PeriodicCallback(_u_ccm, Ts)
    inp = ControlInputs(ccm.us(0))
    NominalProblem(ODEProblem(_nominal_system!, nothing, nothing, (sys, ccm, inp), callback=ccm_f))
end
function nominal_system(sys, ccm, h; Ts = 0.01)
    ccm_f = PeriodicCallback(_u_ccm, Ts)
    inp = ControlInputs(ccm.us(0))
    NominalProblem(ODEProblem(_perturbed_system!, nothing, nothing, (sys, ccm, inp, h), callback=ccm_f))
end
...
function l1_system(sys, ccm, l1, h; Ts = 0.01)
    ccm_f = PeriodicCallback(_u_ccm, Ts)
    inp = ControlInputs(ccm.us(0))
    L1Problem(ODEProblem(_l1_system!, nothing, nothing, (sys, ccm, inp, l1, h), callback=ccm_f))
end
